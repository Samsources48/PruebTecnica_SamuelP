...
namespace Application.Features.Seguridad.Operations
{
    public class UsuariosOperation(IUserRepository userRepository, IRoleRepository roleRepository) : IUsuariosOperation
    {
        ...
        public async Task<UserDto> Create(CreateUserDto dto)
        {
            ...
            //    {
            //        var roled = SeguridadMapper.ToMap(role);
            //        await roleRepository.UpdateAsync(roled);
            //    }
            //}
        ...
        {
            var user = await userRepository.GetByIdAsync(dto.UserId, u => u.Roles)
            ...
            {
                user.Roles.Clear();
                var roles = await roleRepository.GetAllAsync(r => dto.RoleIds.Contains(r.RoleId));
                foreach (var role in roles)
                {
